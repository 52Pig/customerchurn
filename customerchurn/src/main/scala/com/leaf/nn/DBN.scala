...
class DBN(
           ...
           private var momentum: Double,    //动量
...
object DBN extends Serializable {
   ...
   */
  def sigmrnd(P: BDM[Double]): BDM[Double] = {  //编译通过
   ...
   */
  def sigmrnd2(P: BDM[Double]): BDM[Double] = {  //编译通过
    val c = Bexp(P * (-1.0))
    val s1 = 1.0 / (Bexp(P * (-1.0)) + 1.0)
    val r1 = BDM.rand[Double](s1.rows, s1.cols)
    val a3 = s1 + (r1 * 1.0)
   ...
   * X = 1./(1+exp(-P));
   */
  def sigm(matrix: BDM[Double]): BDM[Double] = {  //编译通过
   ...
   * f=1.7159*tanh(2/3.*A);
   */
  def tanh_opt(matrix: BDM[Double]): BDM[Double] = {  //编译通过
    val s1 = Btanh(matrix * (2.0 / 3.0)) * 1.7159
    s1
  }

  def main(args: Array[String]) {
    val c = BDM((1.0, 2.0, 3.0))
    val cc = sigmrnd(c)
    cc.foreachKey(println)
   /* println("+++++++++++++")
    　 　 val dd = sigmrnd(c)
    dd.foreachKey(println)*/
