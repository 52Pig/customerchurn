...
@DeveloperApi
private class MatrixUDT extends UserDefinedType[Matrix] {
...
class DenseMatrix(
  ...
  private def toBreeze: BM[Double] = {
    if (!isTransposed) {
  ...
  private def update(i: Int, j: Int, v: Double): Unit = {
    values(index(i, j)) = v
  }
  ...
  private def map(f: Double => Double) = new DenseMatrix(numRows, numCols, values.map(f),
    isTransposed)

  private def update(f: Double => Double): DenseMatrix = {
...
class SparseMatrix(
  ...
  private def map(f: Double => Double) =
    new SparseMatrix(numRows, numCols, colPtrs, rowIndices, values.map(f), isTransposed)

  private def update(f: Double => Double): SparseMatrix = {
    val len = values.length
    var i = 0
    while (i < len) {
      values(i) = f(values(i))
