...
sealed trait Vector extends Serializable {
  ...
  override def hashCode(): Int = {
    ...
    this.foreachActive { (index, value) =>
      if (nnz < 128 ) {//< Vectors.MAX_HASH_NNZ
...
class DenseVector (
  ...
  def toBreeze: BV[Double] = new BDV[Double](values)
  ...
  override def hashCode(): Int = {
    ...
    var nnz = 0
    while (i < end && nnz < 128 ) { //Vectors.MAX_HASH_NNZ
      val v = values(i)
      if (v != 0.0) {
        result = 31 * result + i
...
class SparseVector  (
  ...
  def toBreeze: BV[Double] = new BSV[Double](indices, values, size)
  ...
  override def hashCode(): Int = {
    ...
    var k = 0
    var nnz = 0
    while (k < end && nnz < 128 ) {  //Vectors.MAX_HASH_NNZ
      val v = values(k)
      if (v != 0.0) {
        val i = indices(k)
        result = 31 * result + i
  ...
  /**
    ...
    */
  private def slice(selectedIndices: Array[Int]): SparseVector = {
    var currentIdx = 0
